...
void RansacPolyfitLine(vector<cv::Point2f> p, int iter_num, float alpha, float* a, float* b)
{
    int r_idx[RANSAC_K];

    vector<cv::Point2f> pick_p;

    srand((unsigned)time(NULL));

    int max_inline_num = 0;

    vector<cv::Point2f> inline_p;
    vector<cv::Point2f> max_inline_p;
    vector<float> d_list;
...
int main(){
    ...
    float a,b;
    RansacPolyfitLine(points,4,0.5,&a,&b);
    cout<<"a= ";
    cout<<a<<endl;
    cout<<"b= ";
    cout<<b<<endl;
